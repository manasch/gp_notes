...
int main()
{
	...
	c3.disp();
	c3 = c1 + 4.0; // operator+(c1, 4.0) => operator+(c1, Complex(c2));
	c3.disp();
	c3 = 4.0 + c1; // operator+(4.0, c1) => operator+(Complex(4.0), c1);
	c3.disp();
	
	// c1 << cout; // can be a member of class Complex; can we support cascading?
	// c1 << "\n" << cout ??? NO
	// put to operator for user defined type?
	cout << c1 << "\n"; // posn of operand is not right to make it a member of Complex
	cout << "\n";
	cout << c2 << "\n";
	cout << "\n";
}
